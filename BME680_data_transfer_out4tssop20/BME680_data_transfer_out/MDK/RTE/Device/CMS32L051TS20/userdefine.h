...
#define TXD0_PORT_SETTING() do{ \
        PORT->P21CFG = 0x05;        /* allocate TXD0 to P21 */ \
        PORT->P2   |=  (1 << 1);    /* P21 output high level */ \
        PORT->PM2  &= ~(1 << 1);    /* P21 is used as TXD0 output */ \
        PORT->POM2 &= ~(1 << 1);    /* P21 is normal output mode */ \
...
#define RXD0_PORT_SETTING() do{ \
        PORT->RXD0PCFG = 0x0d;      /* allocate RXD0 to P50 */ \
        PORT->PM2  |=  (1 << 2);    /* P50 is used as RXD0 input */ \
        PORT->PMC2 &= ~(1 << 2);    /* P50 digital function */ \
}while(0)
...
#define INTP1_PORT_SETTING() do{ \
        PORT->INTP1PCFG = 0x06;     /* allocate INTP1 to P13 */ \
        PORT->PM1  |=  (1 << 3);    /* P13 is used as INTP1 input */ \
        PORT->PMC1 &= ~(1 << 3);    /* P13 digital function */ \
}while(0)

/* ToDo: You can allocate the INTP2 to any desired pins with INTP2PCFG register */
#define INTP2_PORT_SETTING() do{ \
        PORT->INTP2PCFG = 0x07;     /* allocate INTP2 to P14 */ \
        PORT->PM1  |=  (1 << 4);    /* P14 is used as INTP2 input */ \
        PORT->PMC1 &= ~(1 << 4);    /* P14 digital function */ \
}while(0)
